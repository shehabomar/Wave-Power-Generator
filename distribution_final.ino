...
//intiating the process of reading and writing 
  EEPROM.begin(512);
...
//DEFINING THE WIFI MODULE IN THE NODEMCU
...
void loop() {
  // THE FUNCTION THAT HANDELS THE SERVER
  server.handleClient();
...
//SENDING THE RESPONSES RECEIVED FROM NODEMCU
void handle_OnConnect() {
  server.send(200, "text/html", SendHTML(load1,load2,load3,load4)); 
}
//FUNCTIONS CONTROLLING THE LEDS
...
void data(){
  //Stores values from an analog input to the EEPROM.
...
//INFORMING THAT THERE IS NO RESPONSE FOUND
void handle_NotFound(){
  server.send(404, "text/plain", "Not found");
}
//THE HTML FILE THAT WILL BE SENT TO NODEMCU AND DEPLOYED BY NODE'S SERVERS
String SendHTML(uint8_t l1stat,uint8_t l2stat,uint8_t l3stat,uint8_t l4stat){
  String ptr = "<!DOCTYPE html> <html>\n";
  ptr +="<head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, user-scalable=no\">\n";
  ptr +="<title>Water Power Generator</title>\n";
  ptr +="<style>html { font-family: Helvetica; display: inline-block; margin: 0px auto; text-align: center;}\n";
  ptr +="body{margin-top: 160px;background-image: linear-gradient(to top, #86377b 20%, #27273c 80%);} h1 {z-index: 100;color: #444444;margin: 30px auto 10px;} h3 {z-index: 110;color: #444444;margin-bottom: 50px;}\n";
  ptr +=".button {display: block;z-index:10000;width: 80px;background-color: #1abc9c;border: none;color: white;padding: 13px 30px;text-decoration: none;font-size: 25px;margin: 0px auto 35px;cursor: pointer;border-radius: 4px;}\n";
